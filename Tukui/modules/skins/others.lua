...
local function LoadSkin()
	...
	for i = 1, getn(ChatMenus) do
		if _G[ChatMenus[i]] == _G["ChatMenu"] then
			_G[ChatMenus[i]]:HookScript("OnShow", function(self) self:SetTemplate("Skinning", true) self:SetBackdropColor(unpack(C["media"].backdropcolor)) self:ClearAllPoints() self:SetPoint("BOTTOMLEFT", ChatFrame1, "TOPLEFT", 0, T.Scale(30)) end)
		else
			_G[ChatMenus[i]]:HookScript("OnShow", function(self) self:SetTemplate("Skinning", true) self:SetBackdropColor(unpack(C["media"].backdropcolor)) end)
		end
	end
	...
	for i = 1, 4 do
		for j = 1, 3 do
			...
			_G["StaticPopup"..i.."ItemFrame"]:SetTemplate("Skinning")
