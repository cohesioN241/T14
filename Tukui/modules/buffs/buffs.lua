...
local function UpdateWeapons(button, slot, active, expiration)
	if not button.texture then
		...
		button.time:SetPoint(unpack(T.Fonts.bDuration.setoffsets))
...
local function UpdateAuras(header, button, weapon)
	...
	if(not button.texture) then	
		...
		button.count:Point(unpack(T.Fonts.bCount.setoffsets))
...
ScanAuras(CreateAuraHeader("HELPFUL", "TOPRIGHT", -164, -10))
ScanAuras(CreateAuraHeader("HARMFUL", "TOPRIGHT", -164, -130))
