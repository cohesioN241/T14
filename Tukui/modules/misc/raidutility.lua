...
RaidUtilityPanel:CreatePanel("Default", T.Scale(170), panel_height, "TOP", UIParent, "TOP", -350, panel_height + 15)
local r,g,b,_ = C["media"].backdropcolor
RaidUtilityPanel:SetBackdropColor(r,g,b,0.6)
RaidUtilityPanel:CreateShadow("Default")
RaidUtilityPanel:SetFrameLevel(10)
...
local function CreateButton(name, parent, template, width, height, point, relativeto, point2, xOfs, yOfs, text, texture)
	...
	if text then
		...
		t:SetFont(unpack(T.Fonts.dFont.setfont))
		t:SetPoint("CENTER")
...
CreateButton("ShowButton", RaidUtilityPanel, "UIMenuButtonStretchTemplate, SecureHandlerClickTemplate", RaidUtilityPanel:GetWidth() / 2.5, T.Scale(18), "TOP", UIParent, "TOP", -350, 2, "Raid Utility", nil)
ShowButton:SetAttribute("_onclick", [=[
 if select(5, self:GetPoint()) > 0 then
	 self:GetParent():ClearAllPoints()
	 self:GetParent():SetPoint("TOP", UIParent, "TOP", -350, 1)
	 self:ClearAllPoints()
	 self:SetPoint("TOP", UIParent, "TOP", -350, -100)
 else
	 self:GetParent():ClearAllPoints()
	 self:GetParent():SetPoint("TOP", UIParent, "TOP", -350, 500)
	 self:ClearAllPoints()
	 self:SetPoint("TOP", UIParent, "TOP", -350, 1) 
 end
]=])
